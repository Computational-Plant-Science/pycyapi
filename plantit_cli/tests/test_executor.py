...
def test_run_with_directory_input_many_files(remote_base_path, file_name_1, file_name_2):
    with TemporaryDirectory() as temp_dir:
        mock_store = MockStore(temp_dir)
...
def test_run_with_file_output(remote_base_path):
    with TemporaryDirectory() as temp_dir:
        mock_store = MockStore(temp_dir)
...
def test_run_with_directory_output(remote_base_path):
    with TemporaryDirectory() as temp_dir:
        mock_store = MockStore(temp_dir)
...
def test_run_with_file_input_and_directory_output(remote_base_path, file_name_1):
    with TemporaryDirectory() as temp_dir:
        mock_store = MockStore(temp_dir)
...
def test_run_with_directory_output_with_excludes(remote_base_path):
    with TemporaryDirectory() as temp_dir:
        mock_store = MockStore(temp_dir)
        local_output_path = testdir
