...
class Runner(ABC):
    ...
    def __clone_repo(plan: Plan):
        ...
        if plan.branch is not None:
            if subprocess.run(f"git checkout {plan.branch}",
                              ...
                              stderr=subprocess.PIPE,
                              shell=True).returncode != 0:
                raise PlantitException(f"Failed to switch to branch '{plan.branch}'")
    ...
    def __pull_input(self, plan: Plan) -> str:
        ...
        if input_kind == 'directory' or input_kind == 'files':
            self.__store.download_directory(plan.input['from'], input_dir,
                                            plan.input['filetypes'] if 'filetypes' in plan.input else None)
        ...
        if len(input_files) == 0:
            raise PlantitException(f"No inputs found at path '{plan.input['from']}'" + (
                f" matching filetypes '{plan.input['filetypes']}'" if 'filetypes' in plan.input else ''))
    ...
    def __push_output(self, plan: Plan):
        self.__store.upload_directory(
            ...
            plan.output['to'],
            (plan.output['include']['patterns'] if type(
                plan.output['include']['patterns']) is list else None) if 'include' in plan.output and 'patterns' in
                                                                          plan.output['include'] else None,
            (plan.output['include']['names'] if type(
                plan.output['include']['names']) is list else None) if 'include' in plan.output and 'names' in
                                                                       plan.output['include'] else None,
            (plan.output['exclude']['patterns'] if type(
                plan.output['exclude']['patterns']) is list else None) if 'exclude' in plan.output and 'patterns' in
                                                                          plan.output['exclude'] else None,
            (plan.output['exclude']['names'] if type(
                plan.output['exclude']['names']) is list else None) if 'exclude' in plan.output and 'names' in
                                                                       plan.output['exclude'] else None)
