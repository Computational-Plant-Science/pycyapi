...
def test_reduce_csv(tmpdir):
    c = create_db(join(tmpdir, 'results.sqlite'))
    ...
    reduce_csv(c, join(tmpdir, 'results.csv'))

    with open(join(tmpdir, 'results.csv'), 'r') as infile:
        res = infile.read()
        print(res)
        assert res == 'sample,key1,key2\nsample1,val1,val2\nsample2,val3,val4\n'


def test_reduce_csv_missing_keys(tmpdir):
    c = create_db(join(tmpdir, 'results.sqlite'))
...
def test_reduce_csv_key_ordered(tmpdir):
    c = create_db(join(tmpdir, 'results.sqlite'))
    ...
    reduce_csv(c, join(tmpdir, 'results.csv'), key_order=key_order)
...
def test_reduce_file(tmpdir):
    ...
    def add_files(sample_name, files):
        ...
        os.mkdir(join(tmpdir, sample_name))
        for file in files:
            with open(join(tmpdir, sample_name, file), 'w') as fout:
                fout.write(file)
...
def test_no_files(tmpdir):
    ...
    assert not reduce_files(c, tmpdir, tmpdir)


def test_no_key_val(tmpdir):
    sqlite = sqlite3.connect(join(tmpdir, 'results.sqlite'))
    c = sqlite.cursor()
    c.execute('''
              CREATE TABLE samples(
                id INTEGER PRIMARY KEY,
    ...
    assert not reduce_csv(c, join(tmpdir, 'results.csv'))
