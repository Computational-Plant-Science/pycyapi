...
@pytest.fixture
def client(token):
    return AsyncCyverseClient(token)
...
@flaky(max_runs=3)
@pytest.mark.asyncio
async def test_path_exists_when_doesnt(client):
...
@flaky(max_runs=3)
@pytest.mark.asyncio
async def test_path_exists_when_is_a_file(client):
    exists = await client.exists_async(
...
@flaky(max_runs=3)
@pytest.mark.asyncio
async def test_path_exists_when_is_a_directory(client):
    # with trailing slash
    exists = await client.exists_async(
        "/iplant/home/shared/iplantcollaborative/testing_tools/cowsay/"
    )
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_dir_exists_when_is_a_directory(remote_base_path, client, token):
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_dir_exists_when_is_a_file(remote_base_path, client, token):
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_file_exists_when_is_a_file(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file1_name = "f1.txt"
        file2_name = "f2.txt"
        file1_path = join(testdir, file1_name)
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_file_exists_when_is_a_directory(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file1_name = "f1.txt"
        file2_name = "f2.txt"
        file1_path = join(testdir, file1_name)
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_list(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file1_name = "f1.txt"
        file2_name = "f2.txt"
        file1_path = join(testdir, file1_name)
...
@flaky(max_runs=3)
@pytest.mark.asyncio
async def test_list_no_retries_when_path_does_not_exist(remote_base_path, client, token):
    remote_path = join(remote_base_path, str(uuid.uuid4()))
    with pytest.raises(HTTPStatusError) as e:
...
@flaky(max_runs=3)
@pytest.mark.asyncio
async def test_list_retries_when_token_invalid(remote_base_path, client, token):
    remote_path = join(remote_base_path, str(uuid.uuid4()))
    with pytest.raises(HTTPStatusError) as e:
        await client.list_async(remote_path)
        assert "500" in str(e)
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_mkdir(remote_base_path, client, token):
    remote_path = join(remote_base_path, str(uuid.uuid4()))
...
async def test_share(remote_base_path, client, token):
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="todo")
async def test_unshare(remote_base_path, client, token):
    # TODO: how to test this? might need 2 CyVerse accounts
    pass
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_download_file(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file_name = "f1.txt"
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_download_directory(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file1_name = "f1.txt"
        file2_name = "f2.txt"
        file1_path = join(testdir, file1_name)
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_upload_file(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file_name = "f1.txt"
        file_path = join(testdir, file_name)
        remote_path = join(remote_base_path, str(uuid.uuid4()))
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_upload_directory(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file_name1 = "f1.txt"
        file_name2 = "f2.txt"
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_set_metadata(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file_name = "f1.txt"
        file_path = join(testdir, file_name)
        remote_path = join(remote_base_path, str(uuid.uuid4()))
...
@pytest.mark.asyncio
@pytest.mark.slow
async def test_get_metadata(remote_base_path, client, token):
    with TemporaryDirectory() as testdir:
        file_name = "f1.txt"
        file_path = join(testdir, file_name)
        remote_path = join(remote_base_path, str(uuid.uuid4()))
