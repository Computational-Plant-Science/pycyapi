...
class Executor:
    ...
    def execute(sample, workflow, server, results_folder_path):
        ...
        try:
            sample_path = sample.get(working_directory)
        except Exception as err:
            server.update_status(server.WARN,
                                 "Could not get sample %s: %s" % (sample.name, err))
            return

        params_file_path = os.path.join(working_directory, 'params.json')
        params = {
        ...
        if workflow.pre_commands:
            try:
                cmd = [s.format(workdir=working_directory) for s in workflow.pre_commands]
        ...
        try:
            extra_flags = [s.format(workdir=working_directory) for s in workflow.singularity_flags]
            args = [
                       "singularity",
                      "exec"
                      ] + extra_flags + [
            ...
            server.update_status(server.WARN,
                                 (f"Failed to execute workflow '{params_file_path}' on sample '{sample.name}'" +
                                  str(error)))
            return
        ...
        if ret.returncode == 0:
            ...
            if ret.stderr:
                server.update_status(server.WARN,
                                     f"Failed to execute workflow '{params_file_path}' on sample '{sample.name}': {ret.stderr.decode('utf-8')}")
            elif ret.stdout:
                server.update_status(server.WARN,
                                     f"Failed to execute workflow '{params_file_path}' on sample '{sample.name}': {ret.stout.decode('utf-8')}")

            else:
                server.update_status(server.WARN,
                                     f"Failed to execute workflow '{params_file_path}' on sample '{sample.name}': unknown failure")
            return

        if ret.stdout:
            with open("log.out", 'w') as log_file:
                log_file.write(f"{sample.name} stdout: {ret.stdout.decode('utf-8')}")
        if ret.stderr:
            with open("log.err", 'w') as error_log_file:
                error_log_file.write(f"{sample.name} stderr: {ret.stderr.decode('utf-8')}")
