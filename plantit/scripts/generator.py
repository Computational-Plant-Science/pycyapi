...
from plantit.scripts.models import SubmissionConfig, EnvironmentVariable, BindMount
...
class ScriptGenerator:
    ...
    def gen_pull_command(self) -> List[str]:
        ...
        pull_data_command = f"singularity exec {icommands_image} iget -r {self.config.source} {workdir}"
    ...
    def gen_launcher_script(self) -> List[str]:
        ...
        if 'input' in self.config:
            ...
            input_list = [] if ('input' not in self.config or not self.config.input) else \
                [client.stat(input_path)['path'].rpartition('/')[2]] if self.config.parallel else [
                    f['label'] for f in self.client.list_files(input_path)]
    ...
    def gen_singularity_invocation(
        ...
        if bind_mounts is not None and len(bind_mounts) > 0:
            command += (' --bind ' + ','.join([bm.format(work_dir) for bm in bind_mounts]))
